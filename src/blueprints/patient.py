...
# =============================================================================
# INSECURE ENDPOINTS - Kontrollierte Schwachstellen für SAST-Evaluation
# =============================================================================

# ==========================================================================
# SCHWACHSTELLE #3: BSI TR-03161 O.Source_10
# CWE-89: Improper Neutralization of Special Elements used in an SQL Command
# OWASP A05:2025 - Injection
# BESCHREIBUNG: Suchbegriff wird via String-Interpolation in SQL eingebaut.
#               Ermöglicht SQL-Injection mit Eingaben wie "' OR '1'='1".
#               Korrekt wäre: Parameterisierte Query mit Platzhaltern.
# ERWARTETE SAST-ERKENNUNG: SQL injection via string concatenation
# ==========================================================================
@patient_bp.route('/search', methods=['GET'])
@login_required
@require_role(['admin', 'doctor', 'nurse'])
def search_patients_insecure() -> Tuple[Dict[str, Any], int]:
    """
    UNSICHERER Endpoint: SQL-Injection durch String-Interpolation.
    """
    search_term = request.args.get('q', '')

    # SQL-Injection: Suchbegriff wird direkt in Query eingebaut
    from database import get_db_connection
    db = get_db_connection()
    cursor = db.cursor()
    cursor.execute(f"SELECT * FROM patients WHERE last_name LIKE '%{search_term}%'")
...
# ==========================================================================
# SCHWACHSTELLE #1: BSI TR-03161 O.Source_1
# CWE-20: Improper Input Validation
# OWASP A05:2025 - Injection
# BESCHREIBUNG: Benutzereingabe wird ohne Schema-Validierung akzeptiert.
#               Beliebige Datentypen und Längen werden verarbeitet.
#               Korrekt wäre: Validierung mit marshmallow-Schema.
# ERWARTETE SAST-ERKENNUNG: Missing input validation pattern
# ==========================================================================
@patient_bp.route('/<int:patient_id>/note', methods=['POST'])
@login_required
@require_role(['admin', 'doctor', 'nurse'])
def add_note_insecure(patient_id: int) -> Tuple[Dict[str, Any], int]:
    """
    UNSICHERER Endpoint: Akzeptiert Eingabe ohne Validierung.
    """
    # Keine Schema-Validierung - direkte Verwendung von request.json
    note = request.json.get('note')

    execute_insert(
        "INSERT INTO patient_notes (patient_id, note, created_by) VALUES (?, ?, ?)",
        (patient_id, note, g.current_user['id'])
    )
    return jsonify({'message': 'Notiz hinzugefügt'}), 201


# ==========================================================================
# SCHWACHSTELLE #4: BSI TR-03161 O.Source_2
# CWE-79: Improper Neutralization of Input During Web Page Generation
# OWASP A05:2025 - Injection
# BESCHREIBUNG: Patientendaten werden ohne Sanitization ausgegeben.
#               Gespeicherte XSS-Payloads werden unverändert zurückgegeben.
#               Korrekt wäre: sanitize_output() vor JSON-Response.
# ERWARTETE SAST-ERKENNUNG: Missing output encoding/sanitization
# ==========================================================================
@patient_bp.route('/<int:patient_id>/raw', methods=['GET'])
@login_required
@require_role(['admin', 'doctor'])
def get_patient_raw_insecure(patient_id: int) -> Tuple[Dict[str, Any], int]:
    """
    UNSICHERER Endpoint: Gibt Daten ohne Sanitization zurück.
    """
    row = execute_query(
        "SELECT * FROM patients WHERE id = ?",
        (patient_id,),
    ...
    # Keine sanitize_output() - XSS-anfällig
    return jsonify({'name': patient['first_name'] + ' ' + patient['last_name']}), 200


# ==========================================================================
# SCHWACHSTELLE #6: BSI TR-03161 O.Source_4
# CWE-209: Generation of Error Message Containing Sensitive Information
# OWASP A02:2025 - Security Misconfiguration
# BESCHREIBUNG: Stack-Trace wird bei Fehler an Client zurückgegeben.
#               Enthält interne Pfade, Bibliotheksnamen, Codezeilen.
#               Korrekt wäre: Generische Fehlermeldung, Details nur im Log.
# ERWARTETE SAST-ERKENNUNG: Stack trace exposure in error response
# ==========================================================================
@patient_bp.route('/<int:patient_id>/details', methods=['GET'])
@login_required
@require_role(['admin', 'doctor'])
def get_patient_details_insecure(patient_id: int) -> Tuple[Dict[str, Any], int]:
    """
    UNSICHERER Endpoint: Gibt Stack-Trace bei Fehler zurück.
    """
    try:
        row = execute_query(
            "SELECT * FROM patients WHERE id = ?",
        ...
        # Stack-Trace in Response exponiert interne Details
        return jsonify({
            'error': str(e),
...
# ==========================================================================
# SCHWACHSTELLE #26: BSI TR-03161 O.Data_3
# CWE-639: Authorization Bypass Through User-Controlled Key
# OWASP A01:2025 - Broken Access Control
# BESCHREIBUNG: Kein Ownership-Check vor Datenzugriff (IDOR).
#               Jeder authentifizierte User kann alle Patienten abrufen.
#               Korrekt wäre: check_patient_access() vor Datenbankabfrage.
# ERWARTETE SAST-ERKENNUNG: Missing authorization check before data access
# ==========================================================================
@patient_bp.route('/direct/<int:patient_id>', methods=['GET'])
@login_required
def get_patient_direct_insecure(patient_id: int) -> Tuple[Dict[str, Any], int]:
    """
    UNSICHERER Endpoint: Kein Ownership-Check - IDOR möglich.
    """
    # Keine check_patient_access() - jeder authentifizierte User kann alle Patienten sehen
    row = execute_query(
        "SELECT * FROM patients WHERE id = ?",
        (patient_id,),
        fetch_one=True
